...
public class AuthController {
    ...
    public String showLoginForm(@RequestParam(value = "nextUrl", required = false) String nextUrl,
                                ...
                                Model model) {
        if (nextUrl != null && !nextUrl.isEmpty()) {
            // Mock request তৈরি করে SavedRequest object বানানো
            MockHttpServletRequest mockRequest = new MockHttpServletRequest();
            mockRequest.setRequestURI(nextUrl);
            mockRequest.setMethod("GET");
            mockRequest.setServerName("localhost");
            ...
            // DefaultSavedRequest তৈরি (দ্বিতীয় প্যারামিটার হলো contextPath — empty দিলাম)
            DefaultSavedRequest savedRequest = new DefaultSavedRequest(mockRequest, "");

            // session-এ সঠিকভাবে SavedRequest object সেভ করা
            session.setAttribute("SPRING_SECURITY_SAVED_REQUEST", savedRequest);
            model.addAttribute("nextUrl", nextUrl);
            System.out.println("Saved nextUrl as SavedRequest object: " + nextUrl);
        } else {
            session.removeAttribute("SPRING_SECURITY_SAVED_REQUEST");
            System.out.println("No nextUrl - cleared SPRING_SECURITY_SAVED_REQUEST");
        }
